...
exports.createGuestOrderTwo = async (req, res) => {
  const {
    name,
    email,
    password,
  ...
  try {
    ...
    const slot = {
      date: scheduledDateTwo.toISOString(), // Convert date to ISO string
      dayOfWeek: [
        "Sunday",
        "Monday",
        "Tuesday",
...
exports.updateOrderWithImages = async (req, res) => {
  ...
  if (
    !req.files ||
    (req.files.beforeImages.length === 0 && req.files.afterImages.length === 0)
  ) {
  ...
  const beforeImages = req.files.beforeImages
    ? req.files.beforeImages.map((file) => file.path)
    : [];
  const afterImages = req.files.afterImages
    ? req.files.afterImages.map((file) => file.path)
    : [];
