...
const sendVerificationEmail = (email, verificationToken) => {
    ...
    console.log(`Sending verification email to ${email}. Verification Link: ${verificationLink}`);
...
const createUser = async (req, res) => {
    try {
        ...
        return res.json({ status: "SUCCESS", message: "Signup successful. Please verify your email.", data: newUser });
    } catch (error) {
        console.error(error);
        return res.json({ status: "FAILED", message: "An error occurred during user creation." });
    }
};
...
const verifyEmail = async (req, res) => {
    ...
    try {
        ...
        const updatedUser = await User.findOneAndUpdate({ email }, { $set: { isVerified: true } }, { new: true });
...
const updateUser = async (req, res) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(userId, { email, firstName, phone, shippingAddress }, { new: true }); // Corrected property name here

        if (updatedUser) {
            const { password, isVerified, __v, ...userWithoutSensitiveData } = updatedUser.toObject();
            return res.status(200).json({ status: 'SUCCESS', message: 'Profile updated successfully', user: userWithoutSensitiveData });
        } else {
...
const userExists = async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ message: "Internal server error while checking user existence." });
    }
};
...
const isProviderAvailable = async (providerId, date) => {
    try {
        ...
        console.log(`Querying orders from ${startDate.toISOString()} to ${endDate.toISOString()} for provider ${providerId}`);
...
const fetchProviderInfoForOrder = async (req, res) => {
    ...
    try {
        ...
        const availableProviders = await Promise.all(providers.map(async (provider) => {
            const availabilityArray = await isProviderAvailable(provider._id, order.timeSlot.scheduledDate);
...
const reassignProvider = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ message: 'Provider for order changed successfully', order: updatedOrder });
    } catch (error) {
