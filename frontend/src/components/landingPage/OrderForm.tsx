...
const HeadlightRestorationForm = () => {
    ...
    const checkSlotAvailability = (hour, duration) => {
        ...
        if (nextSlotIndex !== -1 && nextSlotIndex + duration <= availableSlots.length) {
    ...
    const fetchAvailableSlots = async () => {
        ...
        try {
            ...
            const availabilityArray = response.data.availability.map(slot => !!slot);
    
            // Store the available slots in state
            const slots = availabilityArray.reduce((acc, available, index) => {
...
useEffect(() => {
    ...
    const totalPrice = (pricePerHeadlight * formData.headlightsCount) + extrasPrice + calculatedPrice;
            ...
            {!formSubmitted ? (
                <>
                    <div className="flex flex-col w-1/3 justify-center items-center mx-auto gap-2 py-10">
                        ...
                        {/* Basic Inputs */}
                        <p>First Name</p>
                        <input type="text" placeholder="First Name" value={formData.name}
                            onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                            className="w-full px-4 py-2 rounded-lg mb-0 text-center appearance-none border border-gray-300" required />
                        <p>Email</p>
                        ...
                        <select id="locationSelect" value={serviceLocation} onChange={(e) => setServiceLocation(e.target.value)}
                            className="w-full px-4 py-2 rounded-lg mb-0 text-center appearance-none border border-gray-300" required>
                            <option value="">Select Location</option>
                            <option value="Garage">Car Medics Oakland Park - 3901 NE 5th Terrace SUITE 4</option>
                            <option value="CustomerLocation">Order service at my location</option>
                        </select>

                        {/* Choose Provider */}
                        <p className="font-semibold">Select Service Provider</p>
                        <select id="providerSelect" value={selectedProvider} onChange={(e) => setSelectedProvider(e.target.value)}
                            className="w-full px-4 py-2 rounded-lg mb-0 text-center appearance-none border border-gray-300" required>
                        ...
                        <div className="flex items-center gap-2 mb-4 mt-2">
                            <button type="button" onClick={handleHeadlightsDecrement} className="px-4 py-2 bg-blue text-white font-bold rounded-lg">-</button>
                            <input type="text" name="headlightsCount"
                                value={formData.headlightsCount} // Set the value attribute to formData.headlightsCount
                                onChange={(e) => setFormData({ ...formData, headlightsCount: e.target.value })} className="text-center w-20 bg-blue/15 text-black px-4 py-2 rounded-lg" />
                            <button type="button" onClick={handleHeadlightsIncrement} className="px-4 py-2 bg-blue text-white font-bold rounded-lg">+</button>
                        </div>
                        ...
                        {serviceLocation === 'CustomerLocation' && (
                            <>
                                <p>Service Charge: <strong>${calculatedPrice.toFixed(2)}</strong></p>
                            </>
                        )}
                        <p>Extras Price: <strong>${extrasPrice.toFixed(2)}</strong></p>
                        <div className="bg-green-300 border border-green-400 rounded-lg p-4 text-center">
                            <p className="font-bold">Total Price:</p>
                            <p className="text-xl font-bold">${totalPrice.toFixed(2)}</p>
                        </div>

                        <p className="mb-2 text-center text-xs">Price includes all taxes and fees and it's final. You will be charged immediately and there is no refunds.</p>
                        <div className="w-full bg-blue/15 px-4 py-4 rounded-lg mb-2">
                            <p className="mb-4 flex flex-row items-center justify-center">Secured <img src={Stripe} className="inline w-12"></img> Payment</p>
                            <CardElement
                                options={{
                                    style: {
                                        base: {
                                        ...
                                        invalid: {
                                            color: '#9e2146', // Text color when the input is invalid
                                            iconColor: '#fa755a', // You can also style the icon color for invalid state
                                        },
                                        // You could also add styles for the 'complete' state if desired
                                        complete: {
                                            color: 'blue', // Example color when the input is correctly filled
                                        },
                                    },
                                }}
                            />
