...
const ProvidersPage = () => {
    ...
    useEffect(() => {
        const fetchProviders = async () => {
            try {
                const { data } = await axios.get('https://carmadices-beta-11pk.vercel.app/user/providers', {
    ...
    const handleClearTimeSlots = async (providerId : any) => {
        ...
        try {
            const response = await axios.delete(`https://carmadices-beta-11pk.vercel.app/admin/${providerId}/clear-time-slots`, {
                headers: { Authorization: `Bearer ${localStorage.getItem('userToken')}` }
            });
