...
const Register: React.FC = () => {
  const [formData, setFormData] = useState<RegisterFormData>({ firstName: '', email: '', password: '', phone: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const errors = validateInput(formData.firstName, formData.email, formData.password, formData.phone);
      ...
      <div className="flex flex-col h-screen">
        ...
        <div className="flex-grow bg-blue/25 lg:flex flex-col items-center justify-center lg:py-10">
          <form onSubmit={handleSubmit} className="bg-white/25 p-8 lg:rounded-2xl">
            <h1 className="text-center text-black text-5xl font-black mb-8">Create an Account</h1>
            ...
            <div className="flex flex-col gap-4 mb-4">
              ...
              <div className="flex flex-col flex-1 text-center font-semibold text-black">
                ...
                {formErrors.firstName && <p className="text-red-500 mt-2">{formErrors.firstName}</p>}
              </div>
              ...
              <div className="flex flex-col flex-1 text-center font-semibold text-black">
                ...
                <input type="email" id="email" name="email" value={formData.email} onChange={handleChange}
                  className="text-black px-4 py-3 rounded-full bg-white" />
                {formErrors.firstName && <p className="text-red-500 mt-2">{formErrors.email}</p>}
              ...
              <div className="flex flex-col flex-1 text-center font-semibold text-black">
                ...
                <input type="password" id="password" name="password" value={formData.password} onChange={handleChange}
                  className="text-black px-4 py-3 rounded-full bg-white" />
                {formErrors.firstName && <p className="text-red-500 mt-2">{formErrors.password}</p>}
              </div>
              <div className="flex flex-col flex-1 text-center font-semibold text-black">
                <label htmlFor="phone" className="mb-2">Phone:</label>
                <input type="text" id="phone" name="phone" value={formData.phone} onChange={handleChange}
                  className="text-black px-4 py-3 rounded-full bg-white" />
                {formErrors.firstName && <p className="text-red-500 mt-2">{formErrors.phone}</p>}

              </div>
            </div>
            {/* Display success or error message */}
            {message && <p className={message.includes('successful') ? 'text-green-600 mb-4 text-center' : 'text-red-600 mb-4 text-center'}>{message}</p>}

            <button type="submit" className="p-3 bg-dark-blue text-white rounded-full w-full">Create Account</button>
            <p className="text-center mt-2">Already have an account? <a href="/login" className="text-blue font-semibold">Log in here.</a></p>
          </form>
        </div>
        <Footer />
      </div>
