...
const ProviderDashboard = () => {
    ...
    const filterOrders = () => {
        const ordersThisMonth = orders.filter(order => isInCurrentMonth(new Date(order.createdAt)));
        const ordersLastMonth = orders.filter(order => isInLastMonth(new Date(order.createdAt)));
        const ordersThisWeek = orders.filter(order => isInCurrentWeek(new Date(order.createdAt)));
        const ordersToday = orders.filter(order => isToday(new Date(order.createdAt)));
        const ordersYesterday = orders.filter(order => isYesterday(new Date(order.createdAt)));
        const ordersThisYear = orders.filter(order => isInCurrentYear(new Date(order.createdAt)));
    ...
    const fetchOrders = async () => {
        try {
            ...
            setLoading(false);
            alert('Failed to fetch orders. Please check your connection and try again.');
        }
    };
    ...
    const completedOrdersWaitingForAdminCheck = filteredOrders.filter(order => order.status === 'Completed' && !order.adminCheck).length;
    const cashForAcceptedOrders = filteredOrders.filter(order => order.adminCheck).reduce((total, order) => total + (order.totalPrice * 0.7), 0).toFixed(2);
    const cashForCompletedButPendingCheck = filteredOrders.filter(order => order.status === 'Completed' && !order.adminCheck).reduce((total, order) => total + (order.totalPrice * 0.7), 0).toFixed(2);
    const cashForPendingOrders = filteredOrders.filter(order => order.status === 'Pending').reduce((total, order) => total + (order.totalPrice * 0.7), 0).toFixed(2);
            ...
            <div className="bg-blue/25 flex-1 overflow-y-auto">
                ...
                <div className="lg:p-8 p-4">
                    <div className="grid lg:grid-cols-6 grid-cols-2 gap-2 mb-4">
                        <button
                            className={`p-4 rounded-md transition-colors duration-300 ${selectedTimePeriod === 'thisMonth' ? 'bg-blue text-white' : 'bg-gray-800 text-white'} hover:bg-blue hover:text-white`}
                            onClick={() => handleTimeRangeChange('thisMonth')}
                        >
                            This Month
                        </button>
                        <button className={`p-4 rounded-md transition-colors duration-300 ${selectedTimePeriod === 'lastMonth' ? 'bg-blue text-white' : 'bg-gray-800 text-white'} hover:bg-blue hover:text-white`}
                            onClick={() => handleTimeRangeChange('lastMonth')}>
                            Last Month
                        </button>
                        <button className={`p-4 rounded-md transition-colors duration-300 ${selectedTimePeriod === 'thisWeek' ? 'bg-blue text-white' : 'bg-gray-800 text-white'} hover:bg-blue hover:text-white`}
                            onClick={() => handleTimeRangeChange('thisWeek')}>
                            This Week
                        </button>
                        <button className={`p-4 rounded-md transition-colors duration-300 ${selectedTimePeriod === 'today' ? 'bg-blue text-white' : 'bg-gray-800 text-white'} hover:bg-blue hover:text-white`}
                            onClick={() => handleTimeRangeChange('today')}>
                            Today
                        </button>
                        <button className={`p-4 rounded-md transition-colors duration-300 ${selectedTimePeriod === 'yesterday' ? 'bg-blue text-white' : 'bg-gray-800 text-white'} hover:bg-blue hover:text-white`}
                            onClick={() => handleTimeRangeChange('yesterday')}>
                            Yesterday
                        </button>
                        <button className={`p-4 rounded-md transition-colors duration-300 ${selectedTimePeriod === 'thisYear' ? 'bg-blue text-white' : 'bg-gray-800 text-white'} hover:bg-blue hover:text-white`}
